-- SQLite Script
-- Mon, Jul 1, 2026
-- Model: recipe_social_media.sql    Version: 3.0
...
CREATE TABLE IF NOT EXISTS Ratings (
    ...
    date_posted DATETIME NOT NULL,
    UNIQUE(Recipes_idRecipes, Users_idUsers),
    FOREIGN KEY (Recipes_idRecipes)
        REFERENCES Recipes(idRecipes)
        ON DELETE CASCADE,
    FOREIGN KEY (Users_idUsers)
...
CREATE TABLE IF NOT EXISTS Media (
    ...
    media_type TEXT NOT NULL CHECK( media_type IN ('recipe', 'ingredient','step' )),
...
CREATE TABLE IF NOT EXISTS RecipeMedia (
    ...
    Recipes_idRecipes INTEGER NOT NULL,
    UNIQUE(Media_idMedia, Recipes_idRecipes),
...
CREATE TABLE IF NOT EXISTS IngredientMedia (
    ...
    Media_idMedia INTEGER NOT NULL,
    Ingredients_idIngredients INTEGER NOT NULL,
    UNIQUE(Media_idMedia, Ingredients_idIngredients),
    FOREIGN KEY (Media_idMedia)
        REFERENCES Media(idMedia)
        ON DELETE CASCADE,
    FOREIGN KEY (Ingredients_idIngredients)
...
-- -----------------------------------------------------
-- Table `RecipeStepMedia`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS RecipeStepMedia (
    idRecipeStepMedia INTEGER PRIMARY KEY AUTOINCREMENT,
    Media_idMedia INTEGER NOT NULL,
    RecipeSteps_idRecipeSteps INTEGER NOT NULL,
    UNIQUE(Media_idMedia, RecipeSteps_idRecipeSteps),
    FOREIGN KEY (Media_idMedia)
        REFERENCES Media(idMedia)
        ON DELETE CASCADE,

    FOREIGN KEY (RecipeSteps_idRecipeSteps)
        REFERENCES RecipeSteps(idRecipeSteps)
        ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_recipestepmedia_step
ON RecipeStepMedia(RecipeSteps_idRecipeSteps);

CREATE INDEX IF NOT EXISTS idx_recipestepmedia_media
ON RecipeStepMedia(Media_idMedia);