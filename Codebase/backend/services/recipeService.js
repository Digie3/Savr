...
export async function getAllRecipes(db, viewerId, sort, order) {

  let orderBy = "datetime(Recipes.date_posted) DESC, Recipes.idRecipes DESC";

  if (sort === "date") {
    orderBy =
      order === "asc"
        ? "datetime(Recipes.date_posted) ASC, Recipes.idRecipes ASC"
        : "datetime(Recipes.date_posted) DESC, Recipes.idRecipes DESC";
  }
  else if (sort === "views") {
    orderBy =
      order === "asc"
    ...
    orderBy =
      order === "asc"
        ? "averageRating IS NULL, averageRating ASC"
        : "averageRating IS NULL, averageRating DESC";
  }
  ...
  return await db.allAsync(`
        SELECT
          ...
          (
            SELECT COUNT(*)
            FROM ActivityEvents
            WHERE ActivityEvents.event_type = 'recipe_view'
              AND ActivityEvents.entity_type = 'recipe'
              AND ActivityEvents.entity_id = CAST(Recipes.idRecipes AS TEXT)
          ) AS viewCount,
          CASE
        ...
        ORDER BY ${orderBy}`, [viewerId, viewerId]);
}
